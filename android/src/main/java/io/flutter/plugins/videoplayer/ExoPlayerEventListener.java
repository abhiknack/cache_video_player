...
public abstract class ExoPlayerEventListener implements Player.Listener {
  ...
  public void onPlayerError(@NonNull final PlaybackException error) {
    ...
    if (error.errorCode == PlaybackException.ERROR_CODE_BEHIND_LIVE_WINDOW) {
        ...
        // Network error - no immediate retry, but store the position
        // so it can resume when the user presses play again
        final long currentPosition = exoPlayer.getCurrentPosition();
        
        // Notify about the error but don't reset the player completely
        events.onError("VideoError", "Network connection interrupted. Tap play to resume.", null);
        
        // The player will remain in error state, but when the user presses play,
        // VideoPlayerPlugin will call play() which will try to recover
    } else {
