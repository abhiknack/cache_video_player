...
final class HttpVideoAsset extends VideoAsset {
  ...
  MediaSource.Factory getMediaSourceFactory(
    ...
    // Add optimal headers for caching
    if (!httpHeaders.containsKey("Range")) {
      httpHeaders.put("Range", "bytes=0-");
    }
    ...
    DataSource.Factory upstreamFactory = new DefaultDataSource.Factory(context, initialFactory);
    
    // Add cache support with optimized flags
    CacheDataSource.Factory cacheDataSourceFactory = new CacheDataSource.Factory()
        .setCache(VideoCacheManager.getCache(context))
        .setUpstreamDataSourceFactory(upstreamFactory)
        .setFlags(CacheDataSource.FLAG_IGNORE_CACHE_ON_ERROR | CacheDataSource.FLAG_BLOCK_ON_CACHE);
